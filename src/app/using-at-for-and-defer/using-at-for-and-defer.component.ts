...
@Component({
    ...
    @for (item of items; track item.id) {
      @defer (on viewport(ul)) {
        ...
        <li tabindex="0" class="tile"><strong>{{ item.id }}: <div class="animate-pulse w-full h-8 bg-gray-300">placeholder</div></strong></li>
      } @loading (minimum 100ms) {
        <li class="tile"><strong>{{ item.id }}: <div class="w-full h-8 bg-red-900">loading</div></strong></li>
      } @error {
